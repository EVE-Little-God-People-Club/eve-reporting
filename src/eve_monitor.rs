...
use crate::image_checker::ImageChecker as _;
use image::RgbaImage;
...
impl EveMonitor {
	pub fn new(char_config: Character) -> anyhow::Result<Self> {
		...
		let n_rgb = &char_config.reminder_point.now_target_rgb;
		let e_rgb = &char_config.reminder_point.enemy_target_rgb;
	...
	fn get_all_point(char_config: &Character) -> (Vec<(u32, u32)>, (u32, u32), (u32, u32)) {
		...
		(
			vec,
			(
				char_config.reminder_point.now.x,
				char_config.reminder_point.now.y,
			),
			(
				char_config.reminder_point.enemy.x,
				char_config.reminder_point.enemy.y,
			),
		)
	}
}
